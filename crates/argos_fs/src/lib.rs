...
//!
//! The crate-wide vocabulary and the checked reads every parser is built on
//! live in this root: each field accessor returns `Option`, so a field that
//! runs off the end of a buffer fails that object's parse instead of
//! panicking (A-UNTRUSTED-ONDISK, A-PARSER-SAFE).
