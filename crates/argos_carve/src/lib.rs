...
//!
//! The validators read the medium through the bounded cursor declared here:
//! every read is capped by an absolute end limit fixed at construction, so a
//! corrupt length field cannot drive a validator past its candidate budget.
