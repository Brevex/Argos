...
//! [`Format`], [`Confidence`] and [`Timestamps`]. That vocabulary is shared by every
//! crate that produces, merges or reports a finding, so a recovery stage and an
//! evidence tier mean exactly one thing across the workspace.
