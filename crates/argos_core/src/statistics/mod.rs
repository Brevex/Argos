...
impl ImageClassifier {
    ...
    pub fn compute_statistics(
        ...
        let gray_values = self.to_grayscale(data, channels);
        ...
        let local_entropy_variance =
            self.compute_local_entropy_variance(&gray_values, width, height);

        let edge_density = self.compute_edge_density(&gray_values, width, height);

        let horizontal_discontinuity =
            self.compute_horizontal_discontinuity(&gray_values, width, height);

        let (color_diversity, distinct_colors) = if channels >= 3 {
            self.compute_color_diversity(data, channels)
        } else {
            (
                gray_values
                    .iter()
    ...
    pub fn is_likely_photo(&self, stats: &ImageStatistics, total_pixels: usize) -> bool {
        self.classify(stats, total_pixels) == ImageClassification::NaturalPhoto
    }

    fn to_grayscale(&self, data: &[u8], channels: usize) -> Vec<u8> {
        match channels {
            1 => data.to_vec(),
            3 => data
                .chunks_exact(3)
    ...
    fn compute_local_entropy_variance(&self, data: &[u8], width: usize, height: usize) -> f64 {
    ...
    fn compute_edge_density(&self, data: &[u8], width: usize, height: usize) -> f64 {
    ...
    fn compute_horizontal_discontinuity(&self, data: &[u8], width: usize, height: usize) -> f64 {
    ...
    fn compute_color_diversity(&self, data: &[u8], channels: usize) -> (f64, usize) {
        ...
        let max_possible = 32 * 32 * 32;

        (count as f64 / max_possible as f64, count)
    }
}
...
mod tests {
    ...
    fn test_kurtosis_uniform() {
        ...
        let (mean, std) = compute_mean_std(&data);
        let kurtosis = compute_kurtosis(&data, mean, std);
        assert!(
            kurtosis < 0.0,
    ...
    fn test_classifier_default() {
        ...
        assert!(config.min_photo_entropy > 0.0);
        assert!(config.max_valid_entropy < 8.0);
    }
    ...
    #[test]
    fn test_classification_photo_check() {
        let classifier = ImageClassifier::new();
        let stats = ImageStatistics {
            entropy: 6.5,
            kurtosis: 1.0,
        ...
        assert!(classifier.is_likely_photo(&stats, 100000));
    }

    #[test]
    fn test_image_classification_methods() {
        assert!(ImageClassification::NaturalPhoto.is_photo());
        assert!(!ImageClassification::ArtificialGraphic.is_photo());
        assert!(ImageClassification::Corrupted.is_corrupted());
        assert!(!ImageClassification::NaturalPhoto.is_corrupted());
    }
    ...
    fn test_grayscale_conversion() {
        ...
        let gray = classifier.to_grayscale(&rgb, 3);
    ...
    fn test_compute_statistics_empty() {
        let classifier = ImageClassifier::new();
        let stats = classifier.compute_statistics(&[], 0, 0, 1);
